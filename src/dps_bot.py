...
def row_from_submitted_value(value: dict) -> dict | None:
    row = {k: str(value.get(k, "")).strip() for k in ("workspace", "catalog", "schema", "table", "access", "message")}
    if not row["catalog"] or not row["schema"]:
        return None
    row["access"] = row["access"] or "Write"
    return row


def _detail_lines(requester_name: str, group_name: str, row: dict, jira_issue: dict | None = None) -> list[str]:
...
def build_access_request_text(owners: list[dict], requester_name: str, group_name: str, row: dict) -> str:
    owner_references = ", ".join(owner_reference(owner) for owner in owners)
    return "\n".join([f"Owners: {owner_references}"] + _detail_lines(requester_name, group_name, row))


def build_owner_request_activity(owners: list[dict], requester_name: str, group_name: str, row: dict, jira_issue: dict):
...
class DPSBot(TeamsActivityHandler):
    async def on_message_activity(self, turn_context: TurnContext) -> None:
        ...
        await turn_context.send_activity(MessageFactory.attachment(CardFactory.adaptive_card(build_request_card())))

    async def _handle_form_submission(self, turn_context: TurnContext, value: dict, action: str) -> None:
        try:
        ...
        if action == "add_row":
            if not current_row:
                await turn_context.send_activity(
                    "Please enter at least catalog and schema before adding another request."
                )
                return
            await turn_context.send_activity(
        ...
        if not rows:
            await turn_context.send_activity(
                "Please enter at least one request (catalog and schema) before submitting."
            )
            return
    ...
    async def _submit_row(self, turn_context: TurnContext, access_token: str, requester: str, row: dict) -> None:
